...
const updateAllMats = () => {
  scene.traverse((child) => {  // 递归遍历每一个 child Material
    if (
...
const renderer = new THREE.WebGLRenderer({
  canvas: canvas,
  antialias: true,  // 是否执行抗锯齿, 默认是 false，抗锯齿意味着， 将分界边缘的像素造成的锯齿进行再分割以消除锯齿感
});
renderer.setSize(sizes.width, sizes.height);
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
...
gui
  ...
  .onFinishChange(() => {
    // JS 会自动将 THREE.ACESFilmicToneMapping 转字符串，但是他们是 Number， 所以强制转下
